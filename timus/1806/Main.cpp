...
using Graph = std::unordered_map<int64_t, std::set<std::pair<int64_t, int>, std::greater<>>>;
using Queue = std::
...
Graph BuildGraph(std::unordered_map<int64_t, int>& indexes, std::vector<int>& weights) {
  Graph map;

  for (auto [v, ind] : indexes) {
    std::set<std::pair<int64_t, int>, std::greater<>> indent;
...
void Dijkstra(std::unordered_map<int64_t, NodeInf>& nodes, Graph& graph, int64_t start) {
  Queue pq;
  nodes[start].ds = 0;
